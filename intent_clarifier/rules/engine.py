...
def try_resolve(question: str) -> Optional[RulesOutcome]:
    ...
    if contains_any(text, FUND_KW):
        entity = Entity.fund
        assumptions.append(
            make_assumption(INFER_ENTITY_FUND, "Entity inferred as fund")
        )

    if contains_any(text, SECURITY_KW) or contains_any(text, HOLDINGS_KW):
        entity = Entity.security
        assumptions.append(
            make_assumption(INFER_ENTITY_SECURITY, "Entity inferred as security")
        )

    if entity is None:
        entity = Entity.fund
        assumptions.append(
            make_assumption(INFER_ENTITY_FUND, "Default entity assumed as fund")
        )
    ...
    if contains_any(text, RETURN_KW):
        ...
        assumptions.append(
            make_assumption(INFER_METRIC_TOTAL_RETURN, "Metric inferred as total return")
        )

    elif contains_any(text, WEIGHT_KW):
        metric = Metric.weight_pct
        assumptions.append(
            make_assumption(INFER_METRIC_WEIGHT_PCT, "Metric inferred as weight percentage")
        )

    elif contains_any(text, SHARES_KW):
        metric = Metric.shares
        assumptions.append(
            make_assumption(INFER_METRIC_SHARES, "Metric inferred as shares")
        )

    else:
        metric = Metric.market_value
        assumptions.append(
            make_assumption(DEFAULT_METRIC_MARKET_VALUE, "Metric defaulted to market value")
        )
    ...
    if contains_any(text, RANK_KW):
        ...
        assumptions.append(
            make_assumption(INFER_ANALYSIS_RANK, "Analysis type inferred as ranking")
        )

    elif contains_any(text, TREND_KW):
        analysis_type = AnalysisType.trend
        assumptions.append(
            make_assumption(INFER_ANALYSIS_TREND, "Analysis type inferred as trend")
        )

    else:
        analysis_type = AnalysisType.snapshot
        assumptions.append(
            make_assumption(INFER_ANALYSIS_SNAPSHOT, "Analysis type inferred as snapshot")
        )
    ...
    if identifiers.fund_key or identifiers.security_key:
        scope = Scope.single
        assumptions.append(
            make_assumption(INFER_SCOPE_SINGLE, "Scope inferred as single entity")
        )
    else:
        scope = Scope.universe
        assumptions.append(
            make_assumption(INFER_SCOPE_UNIVERSE, "Scope inferred as full universe")
        )
    ...
    if metric == Metric.total_return:
        ...
        assumptions.append(
            make_assumption(INFER_TIME_AXIS_MONTH_END, "Time axis inferred as month_end")
        )
    else:
        time_axis = TimeAxis.report_date
        assumptions.append(
            make_assumption(INFER_TIME_AXIS_REPORT_DATE, "Time axis inferred as report_date")
        )
    ...
    if contains_any(text, ALL_YEAR_KW):
    ...
    elif contains_any(text, MOST_RECENT_KW):
        time_window = TimeWindow(mode=TimeWindowMode.most_recent)
        ...
        assumptions.append(
            make_assumption(
                DEFAULT_TIME_WINDOW_MOST_RECENT,
                "Time window defaulted to most recent",
            )
        )
